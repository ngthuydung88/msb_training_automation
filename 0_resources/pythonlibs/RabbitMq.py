...
class RabbitMq(object):
    ...
    def _connect_to_amqp(self, host, port, username='guest', password='guest', alias=None, virtual_host='/',
                         socket_timeout=400, heartbeat_timeout=600, blocked_timeout=300):
        ...
        *Returns:*\n
        ...
        parameters=pika.URLParameters('amqp://{username}:{password}@host:5672/%2F')
        ...
        logger.debug('Connecting using : {params}'.format(params=parameters))
        try:
            self._amqp_connection = BlockedConnection(parameters=parameters)
